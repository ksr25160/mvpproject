...
async def update_item_status(meeting_id: str, item_id: str, status: str):
    ...
    try:
        ...
        logger.log_error_with_context("update action item status failed", e, error_details)
        print(f"❌ 액션 아이템 상태 업데이트 실패: {item_id} - {type(e).__name__}: {str(e)}")
