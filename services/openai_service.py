...
def mask_sensitive_info(text: str) -> str:
    ...
    # 계좌번호 패턴 마스킹
    text = re.sub(r'\d{2,6}[-\s]?\d{2,6}[-\s]?\d{2,6}[-\s]?\d{2,6}', '****-****-****', text)
...
def transcribe_audio(file_path: str) -> str:
    ...
    try:
        ...
        recognizer.session_started.connect(lambda evt: logger.info("음성 인식 세션 시작"))
        recognizer.session_stopped.connect(stop_cb)
        recognizer.canceled.connect(stop_cb)

        recognizer.start_continuous_recognition()
        while not done:
            time.sleep(.5)  # 완료될 때까지 대기
        ...
        log_azure_service_call(logger, "Azure Speech Service", "transcribe_audio", 
                             duration, True, None, f"Transcribed {len(result_text)} characters")
        log_performance(logger, "audio_transcription", duration, f"Output length: {len(result_text)} characters")
        
        logger.info(f"음성 파일 변환 완료: {len(result_text)} 글자")
        return result_text
    except Exception as e:
        duration = time.time() - start_time
        log_error_with_context(logger, e, f"Audio transcription failed for file: {file_path}")
        log_azure_service_call(logger, "Azure Speech Service", "transcribe_audio", 
                             duration, False, None, f"Error: {str(e)}")
...
def summarize_and_extract(text: str) -> dict:
    ...
    try:        # 민감정보 마스킹 적용
        ...
        prompt = f"""
회의록을 읽고 JSON 형식으로 회의 제목(meetingTitle), 회의 요약(summary), 액션 아이템 리스트(actionItems)를 추출하세요.
회의 제목은 회의의 핵심 주제를 5~10단어로 요약하여 생성합니다.
...
각 아이템은 id, description, dueDate(YYYY-MM-DD), recommendedAssigneeId(1~5) 필드를 포함합니다.
담당자 이름이 명확하지 않거나 누락된 경우, 업무 내용·대화 맥락·유사 이름·역할(예: 개발자, QA, 마케터 등)을 기반으로 가장 적합한 담당자를 추론하여 추천하세요.
회의록:
\"\"\"{masked_text}\"\"\"
"""
        ...
        log_azure_service_call(logger, "Azure OpenAI", "chat_completion", 
                             api_duration, True, None, f"Model: {config.AZURE_OPENAI_DEPLOYMENT}")
        ...
        try:
            ...
            log_performance(logger, "meeting_summarization_complete", total_duration, 
            ...
            log_error_with_context(logger, e, f"Invalid JSON response from OpenAI: {content[:200]}...")
        ...
        log_azure_service_call(logger, "Azure OpenAI", "chat_completion", 
                             total_duration, False, None, f"Error: {str(e)}")
