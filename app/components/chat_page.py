...
def render_chatbot(service_manager, fullwidth=False):
    ...
    with chat_container:
        ...
        if not fullwidth:
            ...
            if len(st.session_state.chat_messages) > 5:
                st.caption(
                    f"이전 메시지 {len(st.session_state.chat_messages) - 5}개가 더 있습니다. Chat 탭에서 전체 대화를 확인하세요."
                )
