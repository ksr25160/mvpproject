        ...
        if getattr(st.session_state, "show_add_staff", False):
            with st.expander("➕ 새 직원 정보 입력", expanded=True):
                with st.form("add_staff_form"):
                    ...
                    with col2:
                        email = st.text_input(
                            "이메일 *", placeholder="hong@company.com"
                        )
                    ...
                    with col1:
                        submitted = st.form_submit_button(
                            "👤 직원 추가", use_container_width=True
                        )
                    with col2:
        ...
        try:
            ...
            if staff_list:
                ...
                search_term = st.text_input(
                    "🔍 직원 검색", placeholder="이름, 부서, 직책으로 검색..."
                )
                ...
                if search_term:
                    filtered_staff = [
                        ...
                        if search_term.lower() in staff.get("name", "").lower()
                        or search_term.lower() in staff.get("department", "").lower()
                        or search_term.lower() in staff.get("position", "").lower()
                    ]
                else:
                ...
                for i, staff in enumerate(filtered_staff):
                    with st.container():
                        ...
                        with col2:
                            st.write(f"🏢 {staff.get('department', 'N/A')}")
                        ...
                        with col3:
                            ...
                            if st.button("🗑️ 삭제", key=f"delete_{staff.get('id', i)}"):
                                if st.session_state.get(
                                    f"confirm_delete_{staff.get('id')}", False
                                ):
                                    try:
                                        if service_manager.delete_staff(
                                            staff.get("id")
                                        ):
                                            st.success(
                                                f"✅ '{staff.get('name')}'이 삭제되었습니다!"
                                            )
                                            st.rerun()
                                        else:
                                            st.error("❌ 삭제에 실패했습니다.")
                                    ...
                                    st.session_state[
                                        f"confirm_delete_{staff.get('id')}"
                                    ] = True
                ...
                # 수정 폼
                if (
                    hasattr(st.session_state, "edit_staff_id")
                    and st.session_state.edit_staff_id
                ):
                    edit_staff = st.session_state.edit_staff_data

                    with st.expander(
                        f"✏️ '{edit_staff.get('name')}' 정보 수정", expanded=True
                    ):
                        with st.form("edit_staff_form"):
                            ...
                            with col1:
                                ...
                                new_department = st.selectbox(
                                    ...
                                    index=(
                                        [
                                            ...
                                            "마케팅팀",
                                            "기타",
                                        ].index(edit_staff.get("department", "기타"))
                                        if edit_staff.get("department")
                                        in [
                                            "개발팀",
                                            "기획팀",
                                            "디자인팀",
                                ...
                                new_position = st.text_input(
                                    "직책", value=edit_staff.get("position", "")
                                )

                            with col2:
                                new_email = st.text_input(
                                    "이메일", value=edit_staff.get("email", "")
                                )
                                current_skills = ", ".join(edit_staff.get("skills", []))
                                new_skills_input = st.text_area(
                                    "스킬 (쉼표로 구분)", value=current_skills
                                )

                            col1, col2 = st.columns(2)
                            with col1:
                                if st.form_submit_button(
                                    "✅ 수정 완료", use_container_width=True
                                ):
                                    if (
                                        new_name
                                        and new_department
                                        ...
                                        new_skills = [
                                            skill.strip()
                                            for skill in new_skills_input.split(",")
                                            if skill.strip()
                                        ]
                                        updates = {
                                            "name": new_name,
                                        ...
                                        try:
                                            if service_manager.update_staff(
                                                st.session_state.edit_staff_id, updates
                                            ):
                                                st.success(
                                                    f"✅ '{new_name}' 정보가 수정되었습니다!"
                                                )
                ...
                st.info(
                    "👥 등록된 직원이 없습니다. '더미 데이터 초기화' 버튼을 클릭하거나 새 직원을 추가해보세요."
                )
